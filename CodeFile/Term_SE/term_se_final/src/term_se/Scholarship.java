...
public class Scholarship {
	...
	public boolean Sch_reg(String id, String name, String type, String money) throws IOException {
		...
		**/
		...
		ArrayList tempsss = ul.Std_load();
		String[] temss;
		// 학번 확인
		for(int i = 0 ; i < tempsss.size() ; i++){
			temss = ((String)tempsss.get(i)).split(",");
			
			// 같을 때
			if(id.equals(temss[0])) {
				break;
			}
	...
	public boolean Sch_mod(String id, String name, String type, String money) throws IOException {
		...
		for (i = 0; i < std_sch_list.size(); i++) {
			// 중복시 수정
			if ((std_sch_list.get(i).std_id).equals(id) && (std_sch_list.get(i).std_type).equals(type)) {
